 ...
 */(function($) {
    ...
    if($(window).width() <= 768 && $(window).width() >= 481 ) {
        $('.fl-node-52m8r0fbktuy .pp-heading-separator, .fl-node-52m8r0fbktuy .pp-heading').removeClass('pp-left');
        $('.fl-node-52m8r0fbktuy .pp-heading-separator, .fl-node-52m8r0fbktuy .pp-heading').addClass('pp-tablet-');
    }

    if( $(window).width() <= 480 ) {
        $('.fl-node-52m8r0fbktuy .pp-heading-separator, .fl-node-52m8r0fbktuy .pp-heading').removeClass('pp-left');
        $('.fl-node-52m8r0fbktuy .pp-heading-separator, .fl-node-52m8r0fbktuy .pp-heading').addClass('pp-mobile-');
    }
	...
	$(function() {
		new PPRegistrationForm({
			...
			i18n: {
				messages: {
					error: {
						invalid_username: '이 사용자 이름은 잘못된 문자를 사용하고 있습니다. 올바른 사용자 이름을 입력하세요.',
						username_exists: '이미 등록된 사용자 이름입니다. 다른 이름을 선택하세요.',
						empty_email: '이메일 주소를 입력하세요.',
						invalid_email: '이메일 주소가 정확하지 않습니다!',
						email_exists: '해당 이메일은 이미 등록되어 있습니다. 다른 이메일을 선택해 주세요.',
						password: '비밀번호에는 "\\" 문자가 포함될 수 없습니다.',
						password_length: '비밀번호는 최소 8자 이상이어야 합니다.',
						password_mismatch: '비밀번호가 일치하지 않습니다.',
						invalid_url: 'URL이 유효하지 않은 것 같습니다.',
						recaptcha_php_ver: 'reCAPTCHA API를 사용하려면 PHP 버전 5.3 이상이 필요합니다.',
						recaptcha_missing_key: 'reCAPTCHA 사이트 또는 비밀 키가 없습니다!',
					},
					success: '새로운 사용자가 성공적으로 등록되었습니다!',
				},
				pw_toggle_text: {
					show: '비밀번호 표시',
					hide: '비밀번호 숨기기',
...
jQuery(function($) {
		...
		$(function() {
		$( '.fl-node-iqwoufsjcbe2 .fl-photo-img' )
